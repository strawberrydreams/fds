...
1. **실시간 탐지 (Real-time Detection)**: 거래 요청(`POST /api/v1/transactions`) 시 즉시 FDS 엔진이 개입하여 블랙리스트, 고액 거래, 심야 거래 등을 0.1초 이내에 필터링합니다.
...
* **주요 필드**:
    ...
    * `amount`: 거래 금액
    * `sourceValue` / `targetAccountNumber`: 송금인 / 수취인 계좌번호
...
* `findAllWithUserOrderByCreatedAtDesc()`: **[JPQL]** 거래 내역 조회 시 `User` 엔티티를 `FETCH JOIN`하여 **N+1 문제**를 방지하고 성능을 최적화했습니다.
...
1. **User Request** (`POST /api/v1/transactions`)
